...
describe("resolveConfigDir", () => {
  it("respects LEAKBOOK_CONFIG_DIR override", () => {
    const prev = process.env["LEAKBOOK_CONFIG_DIR"];
    process.env["LEAKBOOK_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["LEAKBOOK_CONFIG_DIR"];
      else process.env["LEAKBOOK_CONFIG_DIR"] = prev;
    }
  });
});
