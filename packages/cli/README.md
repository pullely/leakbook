...
`leakbook` — first-class TypeScript CLI for the Leakbook control
...
This package is internal to the `leakbook` monorepo and is not
...
```
leakbook login    [--api-url=URL] [--token=BEARER]
leakbook logout
leakbook whoami
```
...
```
leakbook org list
leakbook org use <org-id>
leakbook org members
leakbook project list
```

Writes (Task 0101):

```
leakbook org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
leakbook project create <name> [--idempotency-key=KEY]
leakbook env create <project-id> <name> [--idempotency-key=KEY]
leakbook api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
leakbook webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
leakbook usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
leakbook billing summary
leakbook audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`leakbook org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `leakbook login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/leakbook/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/leakbook/config.json` (mode 0644, not a secret). Override
both via `LEAKBOOK_CONFIG_DIR` (used by tests).
