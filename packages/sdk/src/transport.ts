// Base HTTP transport for the Leakbook SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `LeakbookError`
...
export interface ClientOptions {
  /**
   * Base URL of the Leakbook api-edge worker, e.g.
   * `https://api.leakbook.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Leakbook` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Leakbook SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Leakbook SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
