...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Leakbook({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => LeakbookError]> = [
  ...
  it("forward-compat: unknown error code → base LeakbookError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(LeakbookError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as LeakbookError).code).toBe("future_quota_exceeded");
    }
  });
