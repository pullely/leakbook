...
Leakbook TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Leakbook control plane API.

## Install (workspace)

This package is internal to the `leakbook` monorepo. Add it as a
...
```ts
import { Leakbook } from "@saas/sdk";

const client = new Leakbook({
  baseUrl: "https://api.leakbook.app",
  auth: { kind: "bearer", token: process.env["LEAKBOOK_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `LeakbookError`. Branch on
...
| `LeakbookError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `LeakbookError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Leakbook({ fetch })`.
