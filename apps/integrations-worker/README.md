...
Your backend holds one Leakbook API key and **zero GitHub credentials**.
...
const leakbook = new Leakbook({
  baseUrl: process.env.LEAKBOOK_API_URL!,
  token: process.env.LEAKBOOK_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await leakbook.integrations.issueGithubToken(ORG_ID, {
