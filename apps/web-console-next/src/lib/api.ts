...
// deleted; all wire I/O now goes through the typed `Leakbook` resource
...
//   - `createClient` — constructs a `Leakbook` against a target, wired
...
//     `Promise<T>` + thrown `LeakbookError` into the same shape the
...
import { Leakbook, LeakbookError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Leakbook;
...
export function createClient(target: ApiTarget, token: string | null): Leakbook {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Leakbook(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `LeakbookError` subclasses surface as `{ ok: false }` carrying the
